...
TURBO_FLAG(std::string, url, "", "POST this data to the http server");
...
int main(int argc, char* argv[]) {
    ...
    run_app->add_option("-d,--data", FLAGS_d, "POST this data to the http server");
    run_app->add_option("-l,--load_balancer", FLAGS_load_balancer, "The algorithm for load balancing");
    run_app->add_option("-t,--timeout_ms", FLAGS_timeout_ms, "RPC timeout in milliseconds");
    run_app->add_option("-m,--max_retry", FLAGS_max_retry, "Max retries(not including the first RPC)");
    run_app->add_option("-p,--protocol", FLAGS_protocol, "Client-side protocol");
    run_app->add_option_function<bool>("--http_verbose",[](bool v){turbo::set_flag(&FLAGS_http_verbose, v);}, "Print verbose http response");
    run_app->add_option("url", FLAGS_url, "POST this data to the http server")->required();
    auto [ext, code] = turbo::Servlet::instance().run(argc, argv);
    if (ext) {
        return code;
    }
    const std::string url = turbo::get_flag(FLAGS_url).c_str();
    ...
    if (channel.Init(url.c_str(), turbo::get_flag(FLAGS_load_balancer).c_str(), &options) != 0) {
