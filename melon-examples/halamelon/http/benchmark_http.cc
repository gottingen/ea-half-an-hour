...
int main(int argc, char* argv[]) {
    ...
    run_app->add_option("-d,--data", FLAGS_data, "POST this data to the http server");
    run_app->add_option("-t,--thread_num", FLAGS_thread_num, "Number of threads to send requests");
    run_app->add_option_function<bool>("--use_fiber",[](bool v){turbo::set_flag(&FLAGS_use_fiber, v);}, "Use fiber to send requests");
    run_app->add_option("-c,--connection_type", FLAGS_connection_type, "Connection type. Available values: single, pooled, short");
    run_app->add_option("-u,--url", FLAGS_url, "url of server");
    run_app->add_option("-l,--load_balancer", FLAGS_load_balancer, "The algorithm for load balancing");
    run_app->add_option("-o,--timeout_ms", FLAGS_timeout_ms, "RPC timeout in milliseconds");
    run_app->add_option("-m,--max_retry", FLAGS_max_retry, "Max retries(not including the first RPC)");
    run_app->add_option_function<bool>("--dont_fail",[](bool v){turbo::set_flag(&FLAGS_dont_fail, v);}, "Print fatal when some call failed");
    run_app->add_option("-p,--dummy_port", FLAGS_dummy_port, "Launch dummy server at this port");
    run_app->add_option("-r,--protocol", FLAGS_protocol, "Client-side protocol");
    auto [ext, code] = turbo::Servlet::instance().run(argc, argv);
    if (ext) {
        return code;
    }
